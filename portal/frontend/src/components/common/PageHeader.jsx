...
export default function PageHeader({ title, sub, crumbs = [], actions, icon: Icon, iconColor = '#0B74B0' }) {
  return (
    <Box sx={{ mb: 2.5, display: 'flex', flexWrap: 'wrap', gap: 1.5, alignItems: 'flex-end', justifyContent: 'space-between' }}>
      <Box sx={{ display: 'flex', gap: 1.5, alignItems: crumbs.length ? 'flex-end' : 'center' }}>
        {Icon && (
          <Box sx={{ width: 44, height: 44, borderRadius: '12px', flexShrink: 0, display: 'grid', placeItems: 'center',
            bgcolor: iconColor, color: '#fff', boxShadow: `0 6px 14px ${iconColor}55`, mb: 0.25 }}>
            <Icon sx={{ fontSize: 24 }} />
          </Box>
        )}
        <Box>
