...
const Metric = ({ label, value }) => (
  <Box>
    <Typography sx={{ fontFamily: '"IBM Plex Mono",monospace', fontSize: 13, fontWeight: 700 }}>{value}</Typography>
...
export default function MarineServices() {
  ...
  const jobs12m = rows.reduce((s, r) => s + ((r.service && r.service.windowJobs) || 0), 0);
  ...
  return (
    <>
      ...
      <Tabs value={tab} onChange={(_, t) => setTab(t)} sx={{ mb: 2, borderBottom: 1, borderColor: 'divider', minHeight: 40 }}>
      ...
      {tab === 1 && <FleetUtilisation onOpenCraft={(c) => setDetail(rows.find((r) => r._id === c._id) || c)} />}
                      ...
                      <Grid item xs={12} sm={6} md={4} lg={3} key={r._id}>
                        <Card variant="outlined" sx={{ p: 1.75, height: '100%', borderTop: 3, borderTopColor: M2.color }}>
                          ...
                          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
                            {sv.jobs ? `${fmtNum(sv.jobs)} jobs since 2023 · last ${fmtD(sv.lastJobAt)}` : 'No taskings on record'}
                          </Typography>
                          <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
                            <StatusChip value={r.status} map={RESOURCE_STATUS_META} />
                            <Stack direction="row" spacing={0.5}>
                              <Button size="small" startIcon={<HistoryRoundedIcon />} onClick={() => setDetail(r)}>Record</Button>
                              {canEdit && (
                                <Button size="small" onClick={(e) => setMenu({ anchor: e.currentTarget, resource: r })}>Status</Button>
                              )}
                            </Stack>
                          </Stack>
