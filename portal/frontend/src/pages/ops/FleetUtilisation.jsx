...
import {
  Card, Grid, Box, Typography, Stack, Skeleton, Chip, Table, TableHead, TableRow,
  TableCell, TableBody, TableContainer, TextField, MenuItem,
} from '@mui/material';
import {
...
/* Fleet-level utilisation for the marine craft — how much work the tugs,
 * launches, mooring boats and pilots actually did, month by month.
 * Everything is aggregated server-side from the craft service records. */

const TYPE_LABELS = {
  TUG: 'Tugs', PILOT_LAUNCH: 'Pilot launches', MOORING_BOAT: 'Mooring boats',
  PILOT: 'Pilots', SURVEY_LAUNCH: 'Survey launch',
};
...
export default function FleetUtilisation({ onOpenCraft }) {
  ...
  useEffect(() => {
    setData(null);
    api.get('/ops/resources/utilisation', { params: { months } })
  ...
  return (
    <>
      <Grid container spacing={1.5} sx={{ mb: 2 }}>
        ...
        <Grid item xs={6} md={2.4}><Kpi label="Busiest craft" value={data.craft[0] ? data.craft[0].code : '—'} sub={data.craft[0] ? `${fmtNum(data.craft[0].jobs)} jobs · ${data.craft[0].name}` : ''} /></Grid>
        <Grid item xs={12} md={2.4}><Kpi label="Since Jan 2023" value={fmtNum(t.jobsAllTime)} sub={`${fmtNum(t.hoursAllTime)} hours across ${t.craft} craft`} /></Grid>
      </Grid>

      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          <Section
            title="Jobs per month across the fleet" sub="Every berthing, unberthing, pilotage, transfer and line-handling run"
            action={(
              <TextField select size="small" label="Window" value={months} sx={{ minWidth: 130 }}
          ...
          <Section title="Work by craft type" sub={`${t.craft} units on strength`}>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={data.byType} layout="vertical" margin={{ top: 0, right: 20, left: 8, bottom: 0 }}>
                ...
                <YAxis type="category" dataKey="type" width={92} tick={{ fontSize: 11, fill: axis }} axisLine={false} tickLine={false}
        ...
        <Grid item xs={12} md={7}>
          <Section title="Busiest craft" sub="Click a row to open its full service record">
            <TableContainer sx={{ overflowX: 'auto' }}>
              <Table size="small">
                <TableHead><TableRow>
                  <TableCell>Craft</TableCell><TableCell>Type</TableCell>
                  <TableCell align="right">Jobs</TableCell><TableCell sx={{ width: 130 }} />
                  <TableCell align="right">Hours</TableCell><TableCell align="right">Availability</TableCell>
                  <TableCell>Last job</TableCell>
                </TableRow></TableHead>
                <TableBody>
                  {data.craft.map((c) => (
                    <TableRow key={c._id} hover sx={{ cursor: onOpenCraft ? 'pointer' : undefined }}
                      onClick={onOpenCraft ? () => onOpenCraft(c) : undefined}>
                      <TableCell>
                      ...
                      <TableCell sx={{ fontSize: 12 }}>{TYPE_LABELS[c.type] || c.type}</TableCell>
                      <TableCell align="right" sx={{ fontFamily: '"IBM Plex Mono",monospace', fontSize: 12.5 }}>{fmtNum(c.jobs)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell align="right" sx={{ fontFamily: '"IBM Plex Mono",monospace', fontSize: 12.5 }}>{fmtNum(c.hours)}</TableCell>
                      <TableCell align="right" sx={{ fontFamily: '"IBM Plex Mono",monospace', fontSize: 12.5 }}>{c.availabilityPct}%</TableCell>
        ...
        <Grid item xs={12} md={5}>
          <Section title="Jobs by type of movement" sub={`Window ${fmtD(data.window.from)} — ${fmtD(data.window.to)}`}>
              ...
              {data.byKind.map((k) => (
                <Box key={k.kind}>
                  <Stack direction="row" justifyContent="space-between" alignItems="baseline">
                    ...
                    <Typography sx={{ fontFamily: '"IBM Plex Mono",monospace', fontSize: 12 }}>{fmtNum(k.jobs)} · {fmtNum(k.hours)} h</Typography>
                  </Stack>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1.5 }}>
              Fictional demo data, aggregated server-side from the craft service records.
            </Typography>
          </Section>
        </Grid>
      </Grid>
