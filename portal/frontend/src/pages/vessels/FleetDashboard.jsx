...
const TYPE_LABELS = { CONT: 'Container', BULK: 'Bulk carrier', TANK: 'Tanker', GEN: 'General cargo', RORO: 'Ro-Ro', OSV: 'OSV' };
...
export default function FleetDashboard() {
  ...
  useEffect(() => {
    api.get('/vessels/fleet-dashboard').then((r) => setData(r.data))
  ...
  const k = data.kpis;
  const certTotal = data.certs.valid + data.certs.expiring + data.certs.expired;
  ...
  return (
    <>
      ...
      <Grid container spacing={1.5} sx={{ mb: 2 }}>
        ...
        <Grid item xs={6} md={3}><Kpi label="Certificate health" value={certTotal ? `${Math.round((data.certs.valid / certTotal) * 100)}%` : '—'} sub={`${data.certs.expiring} expiring · ${data.certs.expired} expired`} /></Grid>
      </Grid>
          ...
          <Section title="Composition by type">
            <ResponsiveContainer width="100%" height={210}>
              <BarChart data={data.byType} layout="vertical" margin={{ top: 0, right: 24, left: 8, bottom: 0 }}>
                ...
                <YAxis type="category" dataKey="type" width={96} tick={{ fontSize: 11, fill: axis }} axisLine={false} tickLine={false}
        ...
        <Grid item xs={12}>
          <Section title="Vessels needing certificate attention" sub="Expiring within 30 days or already expired — plan renewals and surveys">
            <TableContainer sx={{ overflowX: 'auto' }}>
              <Table size="small">
                <TableHead><TableRow>
                  <TableCell>Vessel</TableCell><TableCell>Type</TableCell><TableCell align="right">Certificates flagged</TableCell><TableCell />
                </TableRow></TableHead>
                <TableBody>
                  {data.certAlertVessels.map((v) => (
                    <TableRow key={v._id} hover>
                      <TableCell><EntityHover type="vessel" id={v._id}><b>{v.name}</b></EntityHover></TableCell>
                      <TableCell>{TYPE_LABELS[v.type] || v.type}</TableCell>
                      <TableCell align="right"><Chip size="small" color="warning" label={v.alerts} sx={{ height: 20, fontWeight: 700 }} /></TableCell>
                      <TableCell align="right">
                        <Button size="small" onClick={() => navigate(`/vessels/${v._id}`)}>Open</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                  {data.certAlertVessels.length === 0 && (
                    <TableRow><TableCell colSpan={4}>
                      <Typography sx={{ py: 3, textAlign: 'center' }} color="text.secondary">Every certificate across the fleet is valid ✅</Typography>
                    </TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
