...
import {
  ...
  TableBody, TableContainer, Divider,
} from '@mui/material';
import {
...
/* One berth's outage record — planned works, breakdowns, dredging and weather
 * stand-downs — and the availability those windows leave. Sample data. */

export const OUTAGE_KIND_META = {
  PLANNED: { label: 'Planned', color: 'info' },
  BREAKDOWN: { label: 'Breakdown', color: 'error' },
  DREDGING: { label: 'Dredging', color: 'warning' },
  WEATHER: { label: 'Weather', color: 'default' },
};
...
export default function BerthOutageDrawer({ berth, onClose }) {
  ...
  useEffect(() => { setData(null); }, [berth?._id]);
  useEffect(() => {
    if (!berth) return;
    api.get(`/berths/${berth._id}/outages`)
  ...
  return (
    <FormDrawer
      open={!!berth} width="64vw" onClose={onClose}
      title={berth ? `${berth.code} — ${berth.name}` : ''}
      subtitle={berth ? `${berth.terminal} · outage history and availability (sample data)` : ''}
    >
      {!data ? <Skeleton variant="rounded" height={380} /> : (
        <>
          <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1.5, flexWrap: 'wrap' }} useFlexGap>
            <StatusChip value={data.berth.status} map={BERTH_STATUS_META} />
            <Chip size="small" variant="outlined" label={data.berth.berthType} sx={{ height: 22, fontSize: 11 }} />
            ...
            <Typography variant="caption" color="text.secondary">
              Record from {fmtD(s.lifetime.firstFrom)} to {fmtD(s.lifetime.lastTo)}
            </Typography>
          </Stack>
          ...
          <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap' }} useFlexGap>
            ...
            <Kpi
              label="Longest window" value={s.longest ? `${s.longest.days} d` : '—'}
              sub={s.longest ? `${s.longest.reason}` : ''}
            />
          </Stack>
          ...
          <Box sx={{ mt: 1.5 }}>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={s.series} margin={{ top: 6, right: 8, left: -22, bottom: 0 }}>
                ...
                <Tooltip contentStyle={tooltipStyle} cursor={{ fill: grid, opacity: 0.35 }} formatter={(v) => `${v} days`} />
                <Bar dataKey="days" name="Days out" fill={C.other} radius={[3, 3, 0, 0]} barSize={18} />
              </BarChart>
            </ResponsiveContainer>
          </Box>
          ...
          <Stack direction="row" spacing={0.75} sx={{ mt: 1, flexWrap: 'wrap' }} useFlexGap>
            {s.byKind.map((k) => (
              <Chip
                ...
                label={`${(OUTAGE_KIND_META[k.kind] || {}).label || k.kind} · ${k.outages} × ${fmtNum(k.days)} d`}
          ...
          <TableContainer sx={{ overflowX: 'auto' }}>
            <Table size="small">
              <TableHead><TableRow>
                ...
                <TableCell align="right">Days</TableCell><TableCell>Type</TableCell>
                <TableCell>Reason</TableCell><TableCell>Carried out by</TableCell>
              </TableRow></TableHead>
              <TableBody>
                {data.outages.map((o) => (
                  <TableRow key={o._id} hover>
                    ...
                    <TableCell>
                      <Chip size="small" color={(OUTAGE_KIND_META[o.kind] || {}).color || 'default'}
                        label={(OUTAGE_KIND_META[o.kind] || {}).label || o.kind} sx={{ height: 20, fontSize: 10.5 }} />
                    </TableCell>
                    <TableCell sx={{ fontSize: 12.5 }}>{o.reason}</TableCell>
                    <TableCell sx={{ fontSize: 12 }}>{o.by || '—'}</TableCell>
                  </TableRow>
                ))}
                {data.outages.length === 0 && (
                  <TableRow><TableCell colSpan={6}>
                    <Typography sx={{ py: 3, textAlign: 'center' }} color="text.secondary">No outage recorded against this berth.</Typography>
                  </TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
            Fictional demo data — maintenance windows are generated for the sample estate.
          </Typography>
        </>
      )}
    </FormDrawer>
