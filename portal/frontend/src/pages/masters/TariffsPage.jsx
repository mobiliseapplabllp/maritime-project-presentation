...
export default function TariffsPage() {
  ...
  return (
    <>
      <CrudPage
        icon={PriceChangeRoundedIcon} iconColor="#BD3861" title="Tariff master"
        sub="Rates applied when invoices are generated from a port call · click a row for its published rate history"
        ...
        columns={[
          ...
          { key: 'rate', label: 'Rate', align: 'right', render: (r) => fmtINR(r.rate), mono: true },
          {
            ...
            label: 'Last revised',
            render: (r) => {
              const rev = lastRevision(r);
              if (!rev) return <Typography variant="caption" color="text.secondary">No history</Typography>;
              return (
                <>
                  <Chip
                    size="small" variant="outlined" color={rev.changePct >= 0 ? 'warning' : 'success'}
                    label={`${rev.changePct >= 0 ? '+' : ''}${rev.changePct}%`}
                    sx={{ height: 20, fontSize: 11, mr: 0.75 }}
                  />
                  <Typography component="span" variant="caption" color="text.secondary" sx={{ whiteSpace: 'nowrap' }}>{fmtD(rev.effectiveFrom)}</Typography>
                </>
              );
            },
          },
        ...
        rowActionsExtra={(row) => (
          <Tooltip title="Rate history">
            <IconButton size="small" onClick={() => setHistory(row)}><TimelineRoundedIcon fontSize="inherit" /></IconButton>
          </Tooltip>
        )}
        formFields={[
